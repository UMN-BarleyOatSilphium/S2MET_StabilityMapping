...
GD <- as.big.matrix(geno_mat_mapping1)
GM <- snp_info %>%
  select(marker, chrom, pos) %>%
  mutate(marker = factor(marker, levels = colnames(GD))) %>%
  arrange(marker) %>%
  as.data.frame()
...
for (i in which(sapply(univariate_gwas_farm_out$gwas_out, is.null))) {
  ...
  Y <- as.data.frame(dat)
  GD1 <- as.big.matrix(GD[levels(Y$line_name),])
  GD2 <- as.big.matrix(t(GD[levels(Y$line_name),]))
...
univariate_gwas_farm_qqplot_list <- univariate_gwas_farm %>%
  mutate(parameter = factor(parameter, levels = c("g", "b", "sd_d"))) %>%
  arrange(trait, parameter) %>%
  mutate(grp = paste(trait, parameter),
         grp = fct_inorder(grp),
         grp = paste0(".id", as.numeric(grp)),
         grp = fct_inorder(grp)) %>%
  split(.$grp)
...
for (i in seq_along(univariate_gwas_farm_qqplot_list)) {
  ...
  trt <- unique(df$trait)
  param <- coef_replace1[unique(df$parameter)]
  subplot <- LETTERS[i]
  
  filename <- paste0("figureS1", subplot, "_GWAS_QQ_", trt, "_", param, ".png")
