...
# Break up the samples by core
K_ns_use_sample_bycore <- split(K_ns_use_sample, sort(rep(seq(n_cores), length.out = length(K_ns_use_sample))))

# Map over cores
var_comp_out <- mclapply(X = K_ns_use_sample_bycore, FUN = function(K_ns_use_sample_list) {
  
  var_comp_list <- vector("list", length(K_ns_use_sample_list))
  
  ## Iterate over the samples
  for (i in seq_along(K_ns_use_sample_list)) {
    
    # Extract the K_matrix
    K_ns_use <- K_ns_use_sample_list[[i]]
    ...
    var_comp_list[[i]] <- fit$var.comp
    
    return(var_comp_list)
