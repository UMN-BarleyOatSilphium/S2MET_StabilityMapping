...
# ## Now estimate stability and the mean using just the VP
# 
# S2_MET_BLUEs_vp <- S2_MET_BLUEs_tpvp %>%
#   filter(line_name %in% vp_geno1)
# 
# # For each list, what are the number of environments and number of unique lines?
# env <- unique(S2_MET_BLUEs_vp$environment)
# n_env <- length(env)
# n_entries <- n_distinct(S2_MET_BLUEs_vp$line_name)
# 
# 
# 
# # Calculate for each trait
# # Also pull out the random effect of each environment
# S2_MET_pheno_mean <- S2_MET_BLUEs_vp %>%
#   group_by(trait) %>%
#   do(calc_gh(.))
# 
# # Ungroup
...
# 
# # Ungroup
# fw_fitted_vp <- ungroup(S2_MET_fw_fitted) %>%
#   # Replace the "no-outlier" group with the original data
#   filter(type == "no_outliers") %>% 
#   bind_rows(., S2_MET_fw_fitted_outliers) %>%
#   arrange(trait, line_name)
# 
# # Extract the data from the outlier-removed model
# pheno_mean_fw_vp <- S2_MET_fw_fitted %>% 
#   filter(type == "no_outliers") %>% 
#   select(trait, line_name, data) %>% 
#   unnest() %>%
#   select(-line_name1, -trait1) %>%
...
# 
# 
# ## What are the correlations between the environmental mean calculating using the TP versus using the VP?
# env_mean_tp <- distinct(pheno_mean_fw, trait, environment, h)
# env_mean_vp <- distinct(pheno_mean_fw_vp, trait, environment, h)
# 
# ## Inner join
# env_mean_join <- inner_join(env_mean_tp, env_mean_vp, by = c("trait", "environment"))
# 
# # Plot
# env_mean_join %>% 
#   ggplot(aes(x = h.x, y = h.y)) +
...
#   xlab("Environmental Mean (TP)") +
#   ylab("Environmental Mean (VP)")
# 
# 
# 
# # Correlation
