...
# Create a relationship matrix using all data
K_all <- A.mat(S2TP_imputed_multi_genos_mat, min.MAF = 0, max.missing = 1)
...
remove_colinear <- function(x) {
  ...
  snp_index <- apply(X = x_LD, MARGIN = 1, FUN = function(LD) max(which(round(LD, 10) == 1)))
