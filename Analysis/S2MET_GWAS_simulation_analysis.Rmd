...
mar_assoc_result <- sim_result_df1 %>% 
  ...
  complete(n_pop, iter, model, sig_level, term, true_false, neg_pos, fill = list(n = 0)) %>%
  select(n_qtl, h2, n_pop:neg_pos, n)

## Summarize qtl discovery results
qtl_disc_result <- sim_result_df1 %>% 
  filter(datatype == "qtl_discovery") %>%
  complete(n_pop, iter, model, sig_level, term, fill = list(n = 0, n_discovered = 0)) %>%
  select(n_qtl, h2, n_pop:neg_pos, n, n_discovered) %>%
...
n_iter <- n_distinct(mar_assoc_result$iter)
...
exp <- expression((sig_level == "fdr0.05") & ( (n_qtl == 30 & h2 == 0.8) | (n_qtl == 100 & h2 == 0.5) ))
...
g_sig_mar <- sig_mar_summ %>% 
  ...
  xlab("Population Size") +
  theme_bw() +
  labs(caption = str_c("n = ", n_iter, " iterations"))
