...
gwas_qtlxe_out1 <- gwas_qtlxe_out %>% 
  gather(trait, p_value, -marker:-pos) %>%
  group_by(trait) %>%
  ...
  mutate(coef = str_replace_all(coef, c(main = "GenotypeMean", qtlxe = "QTLxEnvironment")))

# Q-Q Plot per trait
gwas_qq <- gwas_qtlxe_out1 %>%
  group_by(trait, coef) %>%
