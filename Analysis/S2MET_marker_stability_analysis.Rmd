...
colors <- c("black", umn_palette(n = 4)[-c(1:2)]) %>%
  set_names(c("average", "sensitive", "stable"))
...
Load the GWAS results for genotypic mean and phenotypic stability and see if the markers with the highest plasticity/stability measure overlap
...
grange_combined_overlap_df %>% 
  ...
  group_by(trait) %>% 
  summarize(prop_query_markers = n_distinct(query_markers) / (7 * 2))
