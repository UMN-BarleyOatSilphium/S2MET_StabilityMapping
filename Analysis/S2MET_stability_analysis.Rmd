...
g_pheno_fw_dens <- S2_MET_pheno_fw_uniq_trans %>%
  ...
  facet_wrap(trait ~ term, ncol = 2, scales = "free") +
  xlab("Estimate") +
...
outlier_original_pheno_fw <- S2_MET_pheno_mean_fw %>% 
  ...
  distinct(environment, line_name, trait, g, h, b, delta)

``` 

It doesn't appear that there is anything wrong with these lines. We will consider
these lines when looking at the relationship between phenotype in each environment
and environmental mean
...
pheno_fw_outliers <- S2_MET_pheno_mean_fw_toplot %>% 
  filter(line_name %in% outlier_lines,
         trait == "HeadingDate")

g_pheno_fw_b_outliers <- S2_MET_pheno_mean_fw_toplot %>%
  select(trait, environment, line_name, value, h) %>% 
  filter(trait == "HeadingDate") %>%
  ggplot(aes(x = h, y = value)) + 
  geom_point() + 
  geom_abline(data = pheno_fw_outliers, aes(intercept = g, slope = b, col = line_name), lwd = 1) + 
  scale_color_discrete(guide = guide_legend(title = "Line Name", ncol = 1)) +
  facet_wrap(~ trait, ncol = 1, scales = "free") +
  ylab("Genotype Mean") +
  xlab("Environment Effect") +
  labs(title = "Outlier Genotype Responses") +
  theme_bw() +
  theme(legend.position = "right",
        legend.background = element_rect(fill = "grey85", color = NA),
        legend.text = element_text(size = 8),
...
# Examine the regression objects for the outliers
pheno_fw_outlier_fit <- S2_MET_pheno_mean_fw_toplot %>% 
  filter(line_name %in% outlier_lines,
         trait == "HeadingDate") %>% 
  group_by(trait, line_name) %>% 
  do(fit = lm(value ~ h, data = .))



```

There doesn't seem to be a common issue with these lines. I struggle to find a reason to exclude them.
...
g_stability_and_mean <- stability_mean_corr %>%
  ...
  facet_wrap(trait ~ term, scales = "free", ncol = 2) +
...
Estimate heritability 3 ways:

1. Use a mixed model with a genotype effect and weight the residuals based on the standard error of the estimate of $b_i$ (does not allow modelling of $\delta^2_i$, yet).
2. Mixed model with genotype effect modelled by the relationship matrix and residual genotype effect
...
S2_MET_pheno_sample_fw <- S2_MET_pheno_samples %>%
  unnest() %>%
...
# Save the results
save_file <- file.path(result_dir, "S2_MET_pheno_fw_resampling.RData")
save("S2_MET_pheno_sample_fw", file = save_file)
