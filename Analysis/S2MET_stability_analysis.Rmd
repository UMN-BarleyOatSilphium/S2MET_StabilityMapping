...
S2_MET_pheno_fw %>% 
  select(trait, line_name, stability_term, estimate) %>% 
...
```{r stab.jackknife}
# Extract the BLUEs to model
phenos_tomodel <- S2_MET_pheno_fw %>%
  select(trait, line_name, environment, value, h) %>% 
  distinct()

# Group by trait
stability_repeatability <- phenos_tomodel %>% 
  group_by(trait) %>% 
  do({
    # Extract data
    df <- . 
