  ...
  do({
    ...
    if (nrow(df) == 1) {
      ...
      for (grp in unique(new_grps)) {
        ...
        # If the group is in the new vetor, proceed
        if (grp %in% new_grps1) {
...
sig_loci_df_intervals <- sig_loci_df_groups %>%
  dplyr::rename(group = prelim_group) %>%
  group_by(trait, assoc_test, chrom, group) %>% 
...
n_intervals <- sig_loci_df_intervals %>% 
...
n_intervals %>% 
  ungroup() %>% 
  complete(trait, assoc_test) %>% 
  ggplot(aes(x = trait, y = total_interval, group = assoc_test, fill = assoc_test)) + 
  geom_col(position = "dodge") + 
  scale_fill_discrete(drop = FALSE)

g_n_interval <- n_intervals %>% 
  ungroup() %>% 
  complete(trait, assoc_test) %>% 
  filter(assoc_test %in% c("main_effect_mean", "linear_stability", "non-linear_stability")) %>%
  ggplot(aes(x = trait, y = total_interval, group = assoc_test, fill = assoc_test)) + 
  geom_col(position = "dodge") + 
  scale_fill_discrete(drop = FALSE) +
  theme_bw()
...
ggsave(filename = save_file, plot = g_n_interval)
