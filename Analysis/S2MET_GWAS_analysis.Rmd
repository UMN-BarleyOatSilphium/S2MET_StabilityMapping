...
## Correlation among the values across traits
S2_MET_pheno_mean_fw_trans %>% 
  spread(trait, value) %>% 
  .[,-1] %>% 
  cor()
...
# ## Main effect and QxE
# load(file.path(result_dir, "S2MET_gwas_mean_qxe.RData"))
# 
# # Convert to DF
# gwas_qxe <- gwas_main_qxe %>% 
#   map_df(~mutate(.$scores, model = .$metadata$model)) %>%
#   as_data_frame()
# 
# ## Adjust the p-values
# gwas_geno_qxe_adj <- gwas_qxe %>%
...
#   map(as_data_frame)
# 
# # Calculate an experiment-wise threhold for determining significance
# # For each of N permutations, find the maximum test statistic over all analyses
# gwas_pheno_fw_perm_exp_cutoff <- gwas_pheno_fw_perm %>% 
#   map(~group_by(., trait) %>% mutate(max_test_stat = max(W_statistic))) %>%
#   bind_rows() %>%
#   ungroup()
# 
...
#   full_join(., gwas_pheno_fw_perm_exp_cutoff)
# 
# # Determine the 100(1 - alpha) cutoff across all permutations for a particular trait
# gwas_pheno_fw_perm_sig <- gwas_pheno_fw_perm_ci %>% 
#   group_by(trait) %>% 
#   mutate(test_stat_cutoff = quantile(max_test_stat, prob = (1 - alpha))) %>%
#   ungroup()
...
# Calculate the MAF of all markers
maf <- {colMeans(M + 1) / 2} %>%
  pmin(., 1 - .) %>% 
  data.frame(marker = names(.), maf = ., row.names = NULL, stringsAsFactors = FALSE)

# Combine the minor allele frequency information
gwas_marker_info <- gwas_pheno_mean_fw_adj %>% 
  select(marker, chrom, pos, trait, coef, model, estimate, W_statistic, q_value) %>%
  filter(model == "G") %>%
  unnest() %>%
  left_join(., maf)

# What is the the average maf and effect of significant versus non-significant markers
marker_info_summary <- gwas_marker_info %>%
  mutate(interest = ifelse(q_value <= 0.05, "s", "ns")) %>% 
  group_by(trait, coef, interest) %>% 
  mutate(n_mar = n()) %>%
  summarize_at(vars(estimate, maf, n_mar), funs(mean, sd)) %>% 
  ungroup() %>% 
  complete(trait, coef, interest)

# Plot this
gwas_marker_info %>%
    mutate(interest = ifelse(q_value <= 0.05, "s", "ns")) %>% 
  ggplot(aes(x = abs(maf), fill = interest)) + 
  geom_density() + 
  facet_wrap(coef ~ trait, ncol = 3, scales = "free") + 
  theme_bw()

# Filter out the significant markers
sig_markers <- gwas_marker_info %>% 
