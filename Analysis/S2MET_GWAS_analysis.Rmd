...
trait_BLUEs <- S2_MET_BLUEs_use %>%
  ...
  do({
    ...
    df_use <- df %>%
      ...
      mutate(line_name = `contrasts<-`(x = line_name, value = contr.sum(n = levels(line_name))),
             environment = `contrasts<-`(x = environment, value = contr.sum(n = levels(environment))))
...
# Combine the trait and PCs
trait_pop_str <- left_join(select(A_prcomp_df1, line_name:PC2), trait_BLUEs) 

trait_pop_str %>%
  group_by(trait) %>%
  summarize(trait_popstr1_cor = cor(PC1, value),
            trait_popstr2_cor = cor(PC2, value))

# Plot
trait_pop_str %>% 
  gather(PC, eigen, -line_name:-parent, -trait, -value) %>%
  ggplot(aes(x = eigen, y = value)) +
...
First, association for the main effect of QTL and QTLxE

```{r qtlxe.model}

# Load the data
load(file.path(result_dir, "S2MET_gwas_qtlxe_results.RData"))

# Combine with marker information
gwas_qtlxe_out1 <- left_join(select(s2_imputed_genos, marker = `rs#`, chrom:pos),
                             gwas_qtlxe_out$sig_test)

# Correct p-values for multiple testing
gwas_qtlxe_out2 <- gwas_qtlxe_out1 %>% 
  group_by(trait, term) %>%
  ...
  ungroup() %>%
  mutate(term = str_replace_all(term, c(main_effect = "GenotypeMean", qtl_x_env = "QTLxEnvironment")))

# Q-Q Plot per trait
gwas_qq <- gwas_qtlxe_out2 %>%
  group_by(trait, term) %>%
  arrange(trait, term, p_value) %>% 
  mutate(exp_neg_log_p = seq_along(neg_log_p), 
         exp_neg_log_p = -(log10(exp_neg_log_p / (length(exp_neg_log_p)+1)))) %>%
  arrange(trait, term, chrom, pos)

(g_gwas_qq <- gwas_qq %>%
  ggplot(aes(x = exp_neg_log_p, y = neg_log_p)) +
  geom_point() +
  geom_abline(intercept = 0, slope = 1, col = "red", lwd = 1) +
  facet_grid(trait ~ term) +
