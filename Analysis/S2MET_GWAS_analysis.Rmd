...
gwas_mean_interest_assoc <- gwas_geno_mean_adj %>%
  ...
  select(marker:trait, q_value, local_fdr, interest)
...
# Combine
marker_eff_pheno_fw_interest <- bind_rows(
  mutate(marker_eff_pheno_fw_b_interest, stability_term = "b"),
  mutate(marker_eff_pheno_fw_delta_interest, stability_term = "delta")
)
...
sig_loci_df <- list(list(gwas_mean_interest_assoc, gwas_fw_interest_assoc), 
  ...
  mutate(test = case_when(test == "stable" ~ "marker_stability",
                          test == "sensitive" ~ "marker_plasticity",
                          TRUE ~ test),
         stability_term = case_when(stability_term == "b" ~ "linear",
                                    stability_term == "delta" ~ "non-linear",
                                    TRUE ~ "main_effect")) %>%
  unite(assoc_test, stability_term, test, sep = "_")
...
sig_loci_df %>% 
  group_by(trait, assoc_test, interest) %>% 
  summarize(n = n()) %>% 
  spread(interest, n) %>% 
...
sig_loci_df_groups <- sig_loci_df %>% 
  ...
  group_by(trait, assoc_test, chrom) %>%
  do({
    # Grab the data
    df <- .
...
sig_loci_df_intervals <- sig_loci_df_groups %>% 
  ...
  mutate(y_level = as.numeric(as.factor(assoc_test)))
...
g_sig_windows <- ggplot(data = snp_info, aes(x = pos / 1000000)) + 
  ...
  geom_segment(data = sig_loci_df_intervals, 
               aes(x = interval_start, xend = interval_end, y = y_level, yend = y_level, col = assoc_test),
