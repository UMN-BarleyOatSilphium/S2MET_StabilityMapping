...
save_file <- file.path(fig_dir, "gwas_genotype_mean_qq.jpg")
...
save_file <- file.path(fig_dir, "gwas_genotype_mean_man.jpg")
...
g_man <- gwas_geno_mean_adj %>%
  ...
  scale_color_manual(guide = FALSE, 
                     values = set_names(rep(umn_palette("Secondary_Tier2")[c(3,8)], length.out = 7), seq(1, 7))) +
  scale_linetype_discrete(guide = FALSE) +
  ylab("-log(q)") +
  xlab("Position (Mbp)") +
  labs(title = "Genomewide Assocation Analysis for Train Mean",
       caption = "G model. n = 175 lines used for association analysis.\nThe bold line indicates the genomewide FDR threshold at 5%; the dashed line at 10%.") +
  theme_bw() +
  theme(
    panel.spacing.x = unit(x = 0, units = "in"),
    panel.border = element_blank(),
...
save_file <- file.path(fig_dir, "gwas_pheno_genotype_mean_man_final.jpg")
ggsave(filename = save_file, plot = g_man, width = 9, height = 7, dpi = 1000)
...
# Summarize the q_values
gwas_pheno_fw_qout <- gwas_pheno_fw1 %>%
  group_by(., trait, model) %>% 
  summarize(q_out = list(qvalue(p_value)))

## Quality control
# Inspect the qvalue results

## QQ plot
qq_gwas_pheno_fw <- gwas_pheno_fw1 %>% 
  arrange(., trait, model, p_value) %>% 
  mutate(p_exp = punif(q = seq(0, 1, length.out = n())), 
         neg_log10_p_exp = -log10(p_exp))

# Plot
g_qq <- qq_gwas_pheno_fw_adj %>%
  ggplot(aes(x = neg_log10_p_exp, y = p_value_neg_log10, col = model)) + 
  geom_point() + 
  geom_abline(slope = 1, intercept = 0) + 
  facet_wrap(~ trait, ncol = 2) +
...
gwas_pheno_fw_use <- gwas_pheno_fw_adj$pop_tp %>%
  ungroup() %>%
  filter(model == "G") %>%
  separate(trait, into = c("trait", "stability"), sep = "_") %>%
  mutate(stability = if_else(stability == "b", "Type II", "Type III")) %>% 
  filter(stability == "Type II") %>%
  select(marker:trait, contains("neg_log10_fdr"), Stability = q_value_neg_log10)

# Extract the palette for use in the manhattan plot
palette_use <- umn_palette("Secondary_Tier2")[c(3,8,4,9)]
odd_chrom <- seq(1, 7, by = 2)

gwas_use <- full_join(gwas_geno_mean_use, gwas_pheno_fw_use) %>% 
  gather(test, q_value_neg_log10, -marker:-neg_log10_fdr10) %>% 
...
g_mean_fw_man <- gwas_use %>%
  ...
  labs(title = "Genomewide Assocation Analyses",
       caption = "G model. n = 183 lines used to calculate stability coefficients and n = 175 lines used\nfor association analysis. The bold line indicates the genomewide FDR threshold at 5%;\nthe dashed line at 10%.") +
  theme_bw() +
  theme(
    panel.spacing.x = unit(x = 0, units = "in"),
...
save_file <- file.path(fig_dir, "gwas_mean_and_pheno_fw.jpg")
ggsave(filename = save_file, plot = g_mean_fw_man, width = 9, height = 7, dpi = 1000)
...
gwas_fw_interest_assoc <- gwas_pheno_fw_adj %>%
  ...
  mutate(interest = case_when(q_value_neg_log10 >= neg_log10_fdr05 ~ "significant", 
                              ...
                              TRUE ~ "no_association")) %>%
  filter(interest != "no_association") %>%
  select(marker:stability_term, q_value, local_fdr, interest)

## Establish an outlier cutoff for marker effect stability
# What should be the significance level
alpha <- 0.05

# For each trait, calculate empirical thresholds for significance
marker_eff_pheno_fw_b_interest <- S2_MET_marker_eff_pheno_fw %>%
  filter(stability_term == "b") %>%
  group_by(trait) %>%
  mutate(lower_perc = quantile(estimate, alpha / 2),
         upper_perc = quantile(estimate, 1 - (alpha / 2))) %>%
  ungroup() %>%
  mutate(test = case_when(estimate <= lower_perc ~ "stable",
                          estimate >= upper_perc ~ "sensitive",
                          TRUE ~ "not_significant")) %>%
  select(marker, chrom, pos, trait, test) %>%
  distinct() %>%
  mutate(interest = "significant") %>%
  filter(test != "not_significant")

# Do the same for non-linear stability
marker_eff_pheno_fw_delta_interest <- S2_MET_marker_eff_pheno_fw %>%
  filter(stability_term == "delta") %>%
  group_by(trait) %>%
  mutate(upper_perc = quantile(estimate, 1 - (alpha / 2))) %>%
  ungroup() %>%
  mutate(test = if_else(estimate >= upper_perc, "sensitive", "not_significant")) %>%
  select(marker, chrom, pos, trait, test) %>%
  distinct() %>%
  mutate(interest = "significant") %>%
  filter(test != "not_significant")
...
# gwas_markeff_pheno_fw_adj <- S2_MET_marker_eff_pheno_fw$pop_tp  %>%
#   filter(stability_term == "b") %>%
...
#   mutate_at(vars(starts_with("p_")), funs(qvalue = qvalue(p = .)$qvalue)) %>%
#   mutate_at(vars(contains("qvalue")), funs(neg_log = -log10(.))) %>%
#   mutate(neg_log10_fdr05 = -log10(0.05),
#          neg_log10_fdr10 = -log10(0.10))
# 
# # Test plot
# gwas_markeff_pheno_fw_adj %>% 
#   ggplot(aes(x = pos)) + 
#   geom_point(aes(y = p_gt_zero_qvalue_neg_log)) + 
#   geom_point(aes(y = -p_lt_zero_qvalue_neg_log)) + 
#   geom_hline(aes(yintercept = neg_log10_fdr05, lty = "FDR 05%")) + 
#   geom_hline(aes(yintercept = neg_log10_fdr10, lty = "FDR 10%")) +
#   geom_hline(aes(yintercept = -neg_log10_fdr05, lty = "FDR 05%")) + 
#   geom_hline(aes(yintercept = -neg_log10_fdr10, lty = "FDR 10%")) +
#   facet_grid(trait ~ chrom)
...
# Group the three dfs together
sig_loci_df <- list(list(gwas_mean_interest_assoc, gwas_fw_interest_assoc), 
                    c("mean", "stability")) %>% 
  pmap_df(~mutate(.x, test = .y)) %>%
  bind_rows(., marker_eff_pheno_fw_interest) %>%
  # Rename some tests
  mutate(test = case_when(test == "stable" ~ "marker_stability",
                          test == "sensitive" ~ "marker_sensitivity",
                          TRUE ~ test))

# save
save_file <- file.path(result_dir, "S2MET_genomewide_test_sig_loci.RData")
save("sig_loci_df", file = save_file)

## How many significant or suggestive hits did we get per trait, per test?
sig_loci_df %>% 
  group_by(trait, test, interest) %>% 
  summarize(n = n()) %>% 
  spread(interest, n) %>% 
...
sig_loci_df_groups <- sig_loci_df %>% 
  ...
  do({
    ...
    # Add +/- wind size to each position and assign preliminary groups based on overlap
    df1 <- df %>% 
      mutate(start = pos - wind_size, end = pos + wind_size,
             prelim_group = IRanges(start = start, end = end) %>% 
               findOverlaps(IRanges::reduce(.), .) %>% queryHits() ) %>%
      as.data.frame()
    ...
    if (nrow(df1) == 1) {
      ...
      for (grp in unique(df1$prelim_group)) {
        # Skip if 1
        if (grp == 1) next
      ...
      df1 %>% mutate(group = str_c("group", new_grps)) })
...
## Condense groups into intervals
sig_loci_df_intervals <- sig_loci_df_groups %>% 
  group_by(group, add = T) %>% 
  mutate(interval_start = min(start), interval_end = max(end)) %>%
  ungroup() %>%
  # Assign a y axis value to each test
  mutate(test = factor(test, levels = c("mean", "stability", "marker_stability", "marker_sensitivity")),
         y_level = as.numeric(test))

# How many intervals per chromosome, per trait, per test
sig_loci_df_intervals %>% 
  group_by(trait, test, chrom) %>% 
  summarize(n_interval = n_distinct(group)) %>% 
...
## Plot the windows
g_sig_windows <- ggplot(data = snp_info, aes(x = pos / 1000000)) + 
  geom_segment(aes(x = min(pos  / 1000000), xend = max(pos  / 1000000), y = 0, yend = 0), lwd = 1.5) +
  geom_segment(data = sig_loci_df_intervals, 
...
sig_intervals <- sig_loci_df_intervals %>% 
  ...
  distinct(trait, test, interval_start, interval_end, seqnames) %>%
  group_by(test, trait) %>% 
...
interval_overlaps <- sig_intervals %>% 
  mutate(mean_stability_overlap = list(mean, stability) %>% pmap(find_overlaps),
         stability_mar_stability_overlap = list(stability, marker_stability) %>% pmap(find_overlaps),
         stability_mar_sensitivity_overlap = list(stability, marker_sensitivity) %>% pmap(find_overlaps),
         mar_stability_mar_sensitivity_overlap = list(marker_stability, marker_sensitivity) %>% pmap(find_overlaps) )
